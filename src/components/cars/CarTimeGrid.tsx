...
export default function CarTimeGrid({
		...
		sorted.forEach((res) => {
			...
			for (const group of groups) {
				const overlaps = group.some((existing) => {
					...
					const exStartMin =
						exStart * 60 + parseInt(existing.startTime.split(':')[1]);
					const exEndMin =
						exEnd * 60 + parseInt(existing.endTime.split(':')[1]);
	...
	const reservationPositions = new Map<
		string,
		...
		<div className="relative px-4 py-2">
			...
			<div className="absolute left-16 right-4 top-0">
				...
				{reservations.map((reservation) => {
					...
					const position = reservationPositions.get(reservation.id) || {
						column: 0,
						totalColumns: 1,
					};
